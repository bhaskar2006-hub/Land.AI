...
export const ProfilePage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    fullName: profile?.full_name || 'Government Officer',
    officerId: profile?.id ? profile.id.slice(0, 8).toUpperCase() : 'OFFICER-NIC-01',
    department: profile?.department || DEPARTMENTS[0],
    jurisdiction: JURISDICTIONS[0],
    stateCode: 'TN',
    districtCode: profile?.district || 'NILGIRIS'
  });
  ...
  const activeRole: UserRole = role || 'VIEWER';
  const roleLabel = ROLE_LABELS[activeRole] || 'Public Officer';
  const email = user?.email || profile?.email || 'officer@landai.gov.in';
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1a335a] pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Official government revenue profile, jurisdiction parameters, and database-authenticated RBAC role.
          </p>
        </div>

        {/* Database-Enforced Role Badge */}
        <div className="flex items-center gap-2 bg-[#09152b] px-3 py-1.5 rounded-xl border border-blue-500/30">
          <BadgeCheck size={14} className="text-blue-400" />
          <span className="text-[11px] font-bold text-slate-400">Authenticated Role:</span>
      ...
      {saveSuccess && (
        <div className="p-3 bg-emerald-950/40 border border-emerald-500/50 rounded-xl text-xs text-emerald-300 flex items-center gap-2">
          ...
          <span>Profile changes saved to local session record.</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="glass-panel p-6 text-center space-y-4 border-blue-500/30 bg-gradient-to-b from-[#0a1e3f] via-[#071329] to-[#040b17] shadow-xl">
            ...
            <div>
              <h2 className="text-lg font-black text-white">{formData.fullName}</h2>
              <p className="text-xs text-blue-300 font-mono mt-0.5">{email}</p>
            </div>

            <div className="pt-2 border-t border-[#1a335a] flex justify-center gap-2 flex-wrap">
              {activeRole === 'ADMIN' ? (
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1.5">
                  <Award size={13} className="text-amber-400" /> {roleLabel}
                </span>
              ) : activeRole === 'REVENUE_OFFICER' ? (
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-500/20 text-blue-300 border border-blue-500/30 flex items-center gap-1.5">
                  <Shield size={13} className="text-blue-400" /> {roleLabel}
                </span>
              ) : (
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-500/20 text-slate-300 border border-slate-500/30 flex items-center gap-1.5">
                  <ShieldCheck size={13} className="text-slate-400" /> {roleLabel}
                </span>
              )}
            ...
            <div className="p-3 bg-[#040c1b] rounded-xl border border-[#142d54] text-left space-y-2 text-xs">
              <div className="flex justify-between text-slate-400">
                ...
                <span className="font-mono text-slate-200 font-bold">{formData.officerId}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Department:</span>
                <span className="text-slate-200 text-right font-semibold">{formData.department}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Jurisdiction:</span>
                <span className="text-slate-200 text-right font-semibold">{formData.jurisdiction}</span>
              </div>
            </div>
          ...
          <div className="glass-card p-4 space-y-3 border-blue-500/30">
            <div className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
              <ShieldCheck size={14} className="text-blue-400" /> Role Capabilities ({activeRole})
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              {activeRole === 'ADMIN' ? (
                ...
                <div className="pt-1 text-[11px] text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>Human-in-the-loop (HITL) Validation &amp; Sealing</span>
                </div>
              ) : (
                <div className="pt-1 text-[11px] text-slate-400 flex items-center gap-1.5">
                  <Lock size={12} className="text-slate-500" />
                  <span>Read-only inspection and verification mode</span>
                </div>
              )}
            </div>
          </div>
